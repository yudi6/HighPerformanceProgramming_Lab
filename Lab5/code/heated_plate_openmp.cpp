...
#pragma omp parallel num_threads(4) shared ( w ) private ( i, j )
  {
#pragma omp for
    for ( i = 1; i < M - 1; i++ )
    {
...
*/
#pragma omp parallel num_threads(4) shared ( mean, w ) private ( i, j )
  {
#pragma omp for
    for ( i = 1; i < M - 1; i++ )
    {
...
*/
  ...
  while ( epsilon <= diff )
  {
# pragma omp parallel num_threads(4) shared ( u, w ) private ( i, j )
    {
/*
...
*/
    diff = 0.0;
# pragma omp parallel num_threads(4) shared ( diff, u, w ) private ( i, j, my_diff )
    {
      my_diff = 0.0;
# pragma omp for
      for ( i = 1; i < M - 1; i++ )
